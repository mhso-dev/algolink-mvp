...
```
instructor_fee_per_hour = floor(hourly_rate_krw × instructor_share_pct / 100)
...
business_amount_krw_for_settlement = hourly_rate_krw × total_billed_hours
instructor_fee_krw_for_settlement = instructor_fee_per_hour × total_billed_hours
```
...
- 마이그레이션 (`supabase/migrations/`):
  - `20260429xxxxxx_lecture_sessions.sql` — 신규 테이블 + lecture_session_status enum + 인덱스 + RLS 정책 + soft delete 컬럼
  ...
  - `20260429xxxxxx_settlement_sessions_link.sql` — junction 테이블 + 인덱스
  - (선택) `20260429xxxxxx_project_status_instructor_withdrawn.sql` — `project_status` enum에 `instructor_withdrawn` 값 추가 (SPEC-PROJECT-001 status machine과 협응)
...
- ✅ 산식 정합: `instructor_fee_per_hour = floor(hourly_rate × share_pct / 100)`, `business_amount = hourly_rate × total_hours`, `instructor_fee = fee_per_hour × total_hours` 모두 정수 단위로 일치
...
The system **shall** define a TypeScript module `src/lib/payouts/calculator.ts` exporting four pure functions: `calculateInstructorFeePerHour(hourlyRateKrw: number, sharePct: number): number` returning `Math.floor(hourlyRateKrw * sharePct / 100)`, `calculateTotalBilledHours(sessions: LectureSession[]): number` summing `hours` of sessions with `status === 'completed'`, `calculateBusinessAmount(hourlyRateKrw: number, totalHours: number): number`, `calculateInstructorFee(feePerHour: number, totalHours: number): number`.
...
The unit tests for `calculator.ts` **shall** cover at minimum: (a) `hourly_rate=100000, share_pct=70 → fee_per_hour=70000`, (b) `hourly_rate=80000, share_pct=66.67 → fee_per_hour=53336` (floor), (c) sessions=`[completed:2.0, completed:1.5, planned:1.0, canceled:1.0] → totalHours=3.5`, (d) edge case `share_pct=0 → fee_per_hour=0`, (e) edge case `hourly_rate=0 → fee_per_hour=0`.
...
An index **shall** exist on `lecture_session_id` to support the "is this session already billed?" query used by REQ-PAYOUT002-GENERATE-003.
...
**WHEN** the operator clicks "다른 날로 옮김" on a `planned` session and provides a new date in the dialog, the system **shall** in a single transaction (a) UPDATE the original session's `status` to `'rescheduled'`, (b) INSERT a new lecture_sessions row with the same `project_id`, `instructor_id`, `hours`, `status='planned'`, `original_session_id` set to the original session's id, and the new `date`. The new session **shall** be billable when later marked `completed`.
...
### 4.1 신규 마이그레이션 (3 + 1 선택)

- `supabase/migrations/20260429xxxxxx_lecture_sessions.sql` — `lecture_sessions` 테이블 + `lecture_session_status` enum + 인덱스(`(project_id, date)`, `(instructor_id, date)`) + RLS 정책 3종
...
- `supabase/migrations/20260429xxxxxx_settlement_sessions_link.sql` — `settlement_sessions` junction + 인덱스 + RLS 정책 3종
- (선택) `supabase/migrations/20260429xxxxxx_project_status_instructor_withdrawn.sql` — `project_status` enum에 `instructor_withdrawn` 값 추가 (SPEC-PROJECT-001 협응)
...
### 4.3 기존 도메인 모듈 확장 (SPEC-PAYOUT-001 보존 원칙)
...
CREATE TABLE lecture_sessions (
  ...
  original_session_id uuid REFERENCES lecture_sessions(id) ON DELETE SET NULL,
  notes text,
...
CREATE INDEX idx_settlement_sessions_lecture ON settlement_sessions(lecture_session_id);
...
export function calculateInstructorFeePerHour(hourlyRateKrw: number, sharePct: number): number {
  return Math.floor((hourlyRateKrw * sharePct) / 100);
}
...
- `settlement_sessions` PK가 `(settlement_id, lecture_session_id)`이므로 동일 settlement에 같은 세션을 두 번 link 불가 (DB 제약)
- 두 운영자가 동시에 generate 호출 시: 첫 트랜잭션이 settlement_sessions에 INSERT 완료 후 두 번째 트랜잭션의 SELECT 단계에서 이미 link된 세션은 자동 제외 (이중 청구 방지)

### 5.7 산식 검증 (단위 테스트 케이스 발췌)

| hourly_rate | share_pct | totalHours | expected business | expected fee |
|-------------|-----------|------------|-------------------|--------------|
| 100,000 | 70 | 8.0 | 800,000 | 560,000 |
| 80,000 | 66.67 | 4.5 | 360,000 | 240,012 (53336 × 4.5) |
| 0 | 70 | 8.0 | 0 | 0 |
| 100,000 | 0 | 8.0 | 800,000 | 0 |
| 100,000 | 100 | 8.0 | 800,000 | 800,000 |
...
- ✅ 산식 정합: `instructor_fee_per_hour = floor(hourly_rate × share_pct / 100)` 단위 테스트 PASS
...
| `instructor_withdrawn` enum value 추가가 SPEC-PROJECT-001 status machine과 충돌 | 빌드 에러 | M1 마이그레이션에서 enum 추가 + status machine에 read-only state로 등록. SPEC-PROJECT-001의 7단계 user step 매핑은 `instructor_withdrawn`을 별도 special hold로 처리. |
| lecture_sessions.hours가 numeric(4,1)이라 9999.9 시간까지 허용 | 비현실적 입력 가능 | zod validation에 max(24)로 1일 24시간 제한. 또는 max(720) 월 단위 제한 (운영자 토론 후 결정 — 본 SPEC은 max(24) 권장). |
