...
### 1.1 외부 선행 조건 (HARD prerequisites)
...
- ✅ SPEC-PAYOUT-001 완료 — 4-state 상태 머신 + `validateTransition` + paid-freeze 인배리언트 + `errors.ts` 단일 출처 + `(operator)/settlements/` 라우트. **`client_direct` 흐름은 PAYOUT-001 전환 그래프(`pending → requested → paid` + `held` 분기)를 unchanged 상속**
- ⚠️ **SPEC-PAYOUT-002 (HARD PREREQUISITE — must be merged to main BEFORE starting RECEIPT-001 work)** — 6-1 + sessions 보강 + GENERATE Server Action. SPEC-RECEIPT-001의 `instructor_remittance_amount_krw` 컬럼은 PAYOUT-002의 GENERATE Server Action이 `flow='client_direct'` 정산 행 생성 시 PAYOUT-002의 `instructor_fee_krw`(GENERATED column)에서 derive하여 함께 채운다. PAYOUT-002 미머지 상태에서 RECEIPT-001 작업을 시작하면 강사 송금 등록 단계에서 `instructor_remittance_amount_krw`가 NULL이거나 불일치하여 mismatch error가 발생한다. **사용자 의도 시퀀스: P1(PAYOUT-002) → P2(RECEIPT-001)**.
- ✅ SPEC-ME-001 M5/M7/M8 완료 — `(instructor)/me/payouts/[id]` 정산 상세 페이지 (확장 대상), pgcrypto RPC 패턴 (`encrypt_payout_field`/`decrypt_payout_field`), @react-pdf/renderer + NotoSansKR 폰트 (`public/fonts/NotoSansKR-{Regular,Bold}.ttf`), **LESSON-004 PII GUC + pii_access_log invariant** (REQ-RECEIPT-PII-001에서 사용)
- ✅ SPEC-LAYOUT-001 완료 — UI 프리미티브 11종, 운영자/강사 사이드바
- ✅ Next.js 16 + React 19 + Tailwind 4 + Drizzle 부트스트랩

**Prerequisite gate**: M1 마이그레이션 시작 전, `git log --oneline main | grep -i "SPEC-PAYOUT-002"` 또는 PAYOUT-002 spec.md `status: completed` 확인. 미충족 시 본 SPEC 시작 차단.

### 1.2 본 SPEC 내 선행 조건 (Internal Sequencing)

- M1 (마이그레이션 7건 + Drizzle 스키마 동기화) → 모든 후속 마일스톤의 선행
...
### M1 — 마이그레이션 7건 + Drizzle 스키마 동기화 [Priority: High]

**산출물:**

- 마이그레이션 7건 생성:
  1. `supabase/migrations/20260429000005_app_current_user_role.sql`:
     - `CREATE SCHEMA IF NOT EXISTS app;`
     - `CREATE OR REPLACE FUNCTION app.current_user_role() RETURNS text LANGUAGE sql SECURITY DEFINER SET search_path = public STABLE AS $$ SELECT role::text FROM users WHERE id = auth.uid() $$;`
     - `GRANT EXECUTE ON FUNCTION app.current_user_role() TO authenticated;`
     - **이유**: 본 SPEC의 RLS predicate가 `auth.jwt()->>'role'` (JWT 커스텀 hook 의존) 대신 프로젝트 표준 helper를 사용하기 위함. organization_info, payout-receipts bucket RLS 모두 본 helper를 사용.
  2. `supabase/migrations/20260429000010_settlement_flow_client_direct.sql`:
  ...
  3. `supabase/migrations/20260429000020_settlements_remittance_columns.sql`:
  ...
  4. `supabase/migrations/20260429000030_organization_info.sql`:
     - `CREATE TABLE organization_info` + `CHECK (id = 1)`
     - RLS 정책 (admin RW, operator R) using `app.current_user_role()`
     - 옵션: placeholder seed (TBD 행)
  5. `supabase/migrations/20260429000040_payout_receipts_bucket.sql`:
     - `INSERT INTO storage.buckets (id, name, public, file_size_limit) VALUES ('payout-receipts', 'payout-receipts', false, 52428800);`
     - RLS 정책 using `app.current_user_role()`:
       - `payout_receipts_self_select` (instructor 본인): `bucket_id = 'payout-receipts' AND app.current_user_role() = 'instructor' AND auth.uid() = (SELECT i.user_id FROM instructors i JOIN settlements s ON s.instructor_id = i.id WHERE s.receipt_file_id = (SELECT id FROM files WHERE storage_path = storage.objects.name))`
       - `payout_receipts_operator_all`: `bucket_id = 'payout-receipts' AND app.current_user_role() IN ('operator', 'admin')`
       - default deny (정책 미존재)
  6. `supabase/migrations/20260429000050_notification_type_receipt_issued.sql`:
     - `ALTER TYPE notification_type ADD VALUE IF NOT EXISTS 'receipt_issued';`
  7. `supabase/migrations/20260429000060_receipt_number_counter.sql`:
     - `CREATE TABLE app.receipt_counters (year integer PRIMARY KEY, counter bigint NOT NULL DEFAULT 0)` + ENABLE RLS (no policies, default deny — only SECURITY DEFINER function may access)
     - `CREATE OR REPLACE FUNCTION app.next_receipt_number() RETURNS text LANGUAGE plpgsql SECURITY DEFINER SET search_path = app, public AS $$ ... $$;` (per-year reset via `INSERT ... ON CONFLICT DO UPDATE ... RETURNING`, format `RCP-YYYY-NNNN` with 4-digit pad)
...
- `npx supabase db reset` 후 7개 마이그레이션 모두 정상 적용
- `psql -c "\d+ settlements"` 출력에 6개 신규 컬럼 + UNIQUE 인덱스 확인
- `psql -c "SELECT app.next_receipt_number()"` 호출 시 `RCP-2026-0001` 반환 (4-digit zero-pad)
- 두 번째 호출 시 `RCP-2026-0002` 반환
- 가상 연도 변경 검증: `psql -c "SET TIMEZONE='Asia/Seoul'; UPDATE app.receipt_counters SET counter = 9999 WHERE year = 2026;"` 후 `SELECT app.next_receipt_number()` 호출 → `RCP-2026-10000` (의도적으로 자릿수 초과되는지 확인 — 4자리 형식 위반 시 acceptance에서 잡음)
- helper 검증: `psql -c "SELECT app.current_user_role()"` (anon context에서는 NULL, authenticated context에서는 role 반환)

**연관 EARS:** REQ-RECEIPT-FLOW-001~004, REQ-RECEIPT-COLUMNS-001~002+007, REQ-RECEIPT-NOTIFY-001, REQ-RECEIPT-RLS-001~002
...
- `src/lib/payouts/__tests__/receipt-number.test.ts` (RED):
  - 단일 호출 시 `RCP-YYYY-NNNN` 형식 매칭 (regex: `^RCP-\d{4}-\d{4}$`)
  - 병렬 5건 호출 시 모두 unique (Promise.all + Set 검증)
  - 카운터가 atomic하게 증가하는지 확인 (`0001`, `0002`, `0003`, ...)
  - 연도 reset 검증: `app.receipt_counters` 행 강제로 `year=2027, counter=0` INSERT → 다음 호출 시 (시스템 시각이 2026이지만) 2026 카운터 그대로 진행. 시간 모킹 가능하면 2027 컨텍스트에서 첫 호출 시 `RCP-2027-0001` 반환 검증
...
- `src/lib/payouts/__tests__/receipt-pdf.test.ts` (RED):
  ...
  - 추출 텍스트에 다음 포함:
    - "영수증" (한국어)
    - 영수증 번호 (`RCP-2026-0001`, 4-digit zero-pad)
    - 발행일 (KST 형식)
    - 강사명
...
- `src/lib/payouts/__tests__/operator-confirmation.test.ts` (RED):
  - 정상 케이스: status `requested` + 일치 금액 → atomic 8-step + 1 compensating 모두 성공
    - status → paid
    - receipt_file_id, receipt_number, receipt_issued_at 모두 갱신 (receipt_number 형식 `RCP-YYYY-NNNN`)
    - files 1행 INSERT (kind=`payout_receipt`, storage_path=`<settlement_id>/<receipt_number>.pdf` — bucket-relative, REQ-RECEIPT-COLUMNS-007)
    - notifications 1행 INSERT (type=`receipt_issued`)
    - **pii_access_log 1행 INSERT** (target_table='instructors', target_column='business_number_enc', purpose='receipt_pdf_generation', actor=operator)
    - 콘솔 로그 정확한 형식
  - **PII GUC 검증**: `SET LOCAL app.pii_purpose = 'receipt_pdf_generation'`이 트랜잭션 안에서 호출되었는지 (mock or sql trace 검증)
  - 금액 불일치 → `REMITTANCE_AMOUNT_MISMATCH` 롤백, DB 변경 없음, pii_access_log 변경 없음
  - PDF 생성 실패 (mock 강제) → `RECEIPT_GENERATION_FAILED` 롤백
  - Storage 업로드 실패 (mock) → `STORAGE_UPLOAD_FAILED` 롤백, pii_access_log INSERT도 롤백
  - notifications INSERT 실패 (mock RLS reject) → 전체 롤백 + Storage compensating delete 호출
  - 영수증 재발급 시도 (status=paid에 confirm 호출) → `RECEIPT_ALREADY_ISSUED` 거부
  - Stale (다른 운영자 먼저 처리) → WHERE matched 0 rows → STALE_TRANSITION
  - **decrypt 실패 검증**: instructor.business_number_enc 복호화 RPC가 mock 에러 반환 → 트랜잭션 롤백, pii_access_log INSERT 안 됨
...
- 콘솔 로그 정규식 매칭: `^\[notif\] receipt_issued → instructor_id=[\w-]{36} settlement_id=[\w-]{36} receipt_number=RCP-\d{4}-\d{4}$` (4-digit format)
- pii_access_log INSERT 1행/영수증 검증 (LESSON-004 invariant)
- typecheck + lint 0 에러

**연관 EARS:** REQ-RECEIPT-OPERATOR-001~007, REQ-RECEIPT-NOTIFY-002~004, REQ-RECEIPT-RLS-005, REQ-RECEIPT-PII-001~003
...
- [ ] M1-M7 모든 마일스톤 완료
- [ ] 마이그레이션 7건 정상 적용 (`npx supabase db reset` 무오류)
...
| 위치 | 태그 종류 | 사유 |
|------|----------|------|
| `confirmRemittanceAndIssueReceipt` | `@MX:ANCHOR` | DB-atomic + Storage compensating, fan_in 1 (UI에서만 호출), invariant 8 atomic + 1 compensating step |
| `renderReceiptPdf` | `@MX:NOTE` | @react-pdf/renderer 환경 의존, 폰트 등록 절대 경로 (path.join(process.cwd(), ...)) 필수 |
| `nextReceiptNumber` (RPC 래퍼) | `@MX:ANCHOR` | 동시성 안전 발급 + 연도별 reset, fan_in 1, UNIQUE 제약 의존 |
| `getOrganizationInfo` | `@MX:NOTE` | DB 우선 + env fallback 우선순위 명세 |
| `confirmRemittanceAndIssueReceipt` PII GUC + decrypt 블록 | `@MX:ANCHOR` + `@MX:WARN` | LESSON-004 invariant — `SET LOCAL app.pii_purpose` + `pii_access_log` INSERT 필수, 누락 시 audit 위반 |
| Storage 업로드 후 DB 실패 compensating 처리 | `@MX:WARN` | 고아 파일 발생 가능, best-effort cleanup, REQ-RECEIPT-CLEANUP-001 후속 SPEC 의존 |
| 영수증 발급 후 알림 INSERT | `@MX:NOTE` | SPEC-NOTIFY-001 hook 식별자 (`receipt_issued`) |
| `app.current_user_role()` helper 호출 부분 | `@MX:NOTE` | 프로젝트 표준 RLS helper, JWT 커스텀 hook 의존 제거 |
