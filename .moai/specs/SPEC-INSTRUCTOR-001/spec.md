...
| 해시 | 메시지 |
|------|--------|
| `7a9d888` | feat(instructor): SPEC-INSTRUCTOR-001 — 강사 관리 + AI 만족도 요약 (operator) |
| `b44cdb1` | merge: SPEC-INSTRUCTOR-001 — 담당자용 강사 관리 + AI 만족도 요약 |
| `99aee14` | merge: SPEC-INSTRUCTOR-001 잔여 + SPEC-AUTH-001 v1.1 — user_id auto-mapping |
| `c694845` | feat(auth): SPEC-AUTH-001 v1.1 — accept-invite 시 instructors.user_id 자동 매핑 |

### 구현 완료된 마일스톤

- **M1**: 강사 도메인 타입 (`src/lib/instructor/types.ts`)
- **M2**: 리스트 쿼리 — `listInstructorsForOperator` single-query aggregation + format 헬퍼 (`src/lib/instructor/queries.ts`, `format.ts`)
- **M3**: 상세 쿼리 — `getInstructorDetailForOperator`, `getRecentReviewComments`
- **M4**: zod validation (`src/lib/validation/instructor.ts`)
- **M5**: AI 만족도 요약 — `getOrGenerateSummary`, 24h 캐시, fallback, PII 분리, Prompt Caching (`src/lib/ai/instructor-summary.ts`, `instructor-summary-server.ts`)
- **M6**: UI 컴포넌트 — `instructor-list-table`, `instructor-list-filters`, `satisfaction-summary-card`, `instructor-history-table`, `instructor-form`, `pagination`, `regenerate-button`
- **M7**: 페이지 + Server Actions — `/instructors`, `/instructors/[id]`, `/instructors/new`

### Divergence 분석

| 카테고리 | 내용 |
|---------|------|
| **unplanned_additions** | `src/lib/ai/anthropic-client.ts` 신규 생성 (SPEC §4.2 `src/lib/ai/anthropic-client.ts` 명시 — 사실상 scope 내). `src/lib/instructor/link-user.ts` 추가 — `instructors.user_id` 자동 매핑 로직 분리 (SPEC-AUTH-001 v1.1로 위임했으나 별도 모듈로 구현). |
| **structural_changes** | SPEC에서 `src/lib/instructor/skills.ts` 명시했으나 구현 미포함 (skill multiselect는 등록폼에서 인라인 처리). `satisfaction-range-slider.tsx` 미구현 (만족도 범위는 텍스트 입력으로 대체). |
| **deferred_items** | AI 만족도 정확도 운영자 수동 검증 3건 (acceptance.md 게이트) — 운영 데이터 필요, 운영 단계로 이연. |
